...
#'
#' @return A list of two data frames: \code{$potential} and \code{$logistic}, each containing results for CRM at n_initial = 1:5 and 3+3 with n_initial = 3.
#' @export
#' @examples
#' result_list <- simulate_across_n_initial()
#' head(result_list$potential)
simulate_across_n_initial <- function(num_rep = 1000, seed = 1234) {
  # ----------------------------
  # POTENTIAL MODEL
  # ----------------------------
  crm_potential_results <- lapply(1:5, function(n_init) {
  ...
  df_potential <- df_potential[, c("method", "n_initial",
  ...
  # LOGISTIC MODEL
  # ----------------------------
  crm_logistic_results <- lapply(1:5, function(n_init) {
  ...
  df_logistic <- df_logistic[, c("method", "n_initial",
                                 "mean_mtd", "var_mtd", "median_mtd", "iqr_mtd",
                                 "mean_tox", "var_tox", "median_tox", "iqr_tox")]
